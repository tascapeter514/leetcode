...
# recursive
def convert(starting_number, reach, target):
    if starting_number >= target:
        return
    else:
        starting_number * convert(starting_number, reach, target)


        

print(mormons(test1['input']['starting_number'], test1['input']['reach'], test1['input']['target']))
for test in tests:
    print(mormons(test['input']['starting_number'], test['input']['reach'], test['input']['target']) == test['output'])